 ...
 */
export const SEKAR_JAWA_3D_JOURNEY = [
  {
    id: "cover",
    range: [0, 0.07],
...
export function createJourneyCameraTransform(): JourneyCameraTransform {
  const first = SEKAR_JAWA_3D_JOURNEY[0].camera;
  return {
...
export function getJourneyTargetProgress(sectionType: string) {
  const stop = SEKAR_JAWA_3D_JOURNEY.find(
...
export function writeJourneyCameraTransform(
  ...
  const lastIndex = SEKAR_JAWA_3D_JOURNEY.length - 1;
  let fromIndex = lastIndex;

  for (let index = 0; index < lastIndex; index++) {
    if (p < SEKAR_JAWA_3D_JOURNEY[index + 1].range[0]) {
      fromIndex = index;
      break;
    }
  }

  const from = SEKAR_JAWA_3D_JOURNEY[fromIndex];
  const to = SEKAR_JAWA_3D_JOURNEY[Math.min(fromIndex + 1, lastIndex)];
