...
const template = getTemplate("sekar-jawa-3d")!;
const sections = () => hydrateTemplateSections(template);

describe("Sekar Jawa 3D functional wiring", () => {
  ...
  it("exposes the required Sekar Jawa 3D journey destinations without changing the shared dock default", () => {
  ...
  it("normalizes all configured events and derives hero/countdown from the first event", () => {
    ...
    expect(
      sekarJawa3DSectionProps(countdown, content.events, content.eventDate)
        .target_date,
    ).toBe("2040-06-01");
  ...
  it("keeps the same functional DOM available in immersive and Simple modes", () => {
    ...
    expect(sekarJawa3DContent(input).remaining.map((s) => s.type)).toEqual(
