...
export function EventSekarJawa3D({ props }: SectionRenderProps) {
  const p = props as { intro?: string; events?: unknown };
  const events = normalizeEventDetails(p.events);
  return (
    <section className="px-5 py-8 text-center">
      <p
        data-ceremony-reveal="0"
        className="text-xs tracking-[0.3em] text-[var(--inv-secondary)] uppercase"
      >
        Rangkaian Acara
      </p>
      {p.intro ? (
      ...
      <div className={styles.royalEventGrid}>
        {events.map((event, index) => (
          <article
            data-ceremony-reveal={160 + Math.min(index, 4) * 100}
